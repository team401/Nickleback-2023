...
public class DriveSubsystem extends SubsystemBase{
    ...
    public DriveSubsystem() {
        ...
        Supplier<ChassisSpeeds> speedsSupplier = () -> this.getChassisSpeeds();
        Consumer<ChassisSpeeds> outputRobotRelative = chassisSpeeds -> this.setGoalChassisSpeeds(chassisSpeeds);
        Consumer<Pose2d> poseResetter = newPose -> RobotState.getInstance().resetOdometry(new Rotation2d(0), this.getSwerveModulePositions(), newPose);

        AutoBuilder.configureHolonomic(
                    poseSupplier, // Robot pose supplier
                    poseResetter, // Method to reset odometry (will be called if your auto has a starting pose
                    speedsSupplier, // ChassisSpeeds supplier. MUST BE ROBOT RELATIVE
                    outputRobotRelative, // Method that will drive the robot given ROBOT RELATIVE ChassisSpeeds
