...
public class DriveSubsystem extends SubsystemBase {
    ...
    private final DriveIO[] driveModules = new DriveIO[4]; // TODO: This will be the future format: FL, FR, BR, BL
    private final AngleIO pigeonInterface;
    ...
    public DriveSubsystem() {
        if (Constants.isReal) {
            driveModules[0] = new SwerveModules(DriveConstants.frontLeftDriveID,
            ...
            driveModules[1] = new SwerveModules(DriveConstants.frontRightDriveID,
            ...
            driveModules[2] = new SwerveModules(DriveConstants.backRightDriveID,
            ...
            pigeonInterface = new PigeonInterface();
        } else {
            driveModules[0] = new ModuleSim();
            driveModules[1] = new ModuleSim();
            driveModules[2] = new ModuleSim();
            driveModules[3] = new ModuleSim();

            pigeonInterface = new AngleIO();
        }
    ...
    public void periodic() {
        for (int i = 0; i < 4; i++) {
            ...
            SwerveModuleState optimizedState = SwerveModuleState.optimize(driveModules[i].getModuleState(),
                    moduleRotation);
        ...
        for (int i = 0; i < 4; i++) {
            driveModules[i].getModulePosition().distanceMeters = driveModules[i].getDrivePosition()
                    * DriveConstants.wheelRadiusM;
            driveModules[i].getModulePosition().angle = new Rotation2d(driveModules[i].getRotationPosition());

            SmartDashboard.putNumber("Drive/modules/" + i + "/angle",
                    driveModules[i].getModulePosition().angle.getRadians());
            SmartDashboard.putNumber("Drive/modules/" + i + "/drive stator current",
                    driveModules[i].getDriveStatorCurrent());
            SmartDashboard.putNumber("Drive/modules/" + i + "/rotation stator current",
                    driveModules[i].getRotationStatorCurrent());
        }
        ...
        SmartDashboard.putNumber("Encoder Position 1",
                new Rotation2d(driveModules[0].getRotationPosition()).getRotations());
        SmartDashboard.putNumber("Encoder Position 2",
                new Rotation2d(driveModules[1].getRotationPosition()).getRotations());
        SmartDashboard.putNumber("Encoder Position 3",
                new Rotation2d(driveModules[2].getRotationPosition()).getRotations());
        SmartDashboard.putNumber("Encoder Position 4",
                new Rotation2d(driveModules[3].getRotationPosition()).getRotations());
    }
    ...
    /**
     * Set the desired velocities of the robot drive (xVelocity, yVelocity,
     * omegaVelocity)
     * 
     * @param speeds the desired speed of the robot
     */
    public void setGoalChassisSpeeds(ChassisSpeeds speeds) {
        speeds = new ChassisSpeeds(speeds.vxMetersPerSecond * (babyMode ? 0.2 : 1),
                speeds.vyMetersPerSecond * (babyMode ? 0.2 : 1), speeds.omegaRadiansPerSecond * (babyMode ? 0.1 : 1));
